...
export _CSD_1_Stop
export  CSD_1_Stop
export _CSD_1_SetScanMode
export  CSD_1_SetScanMode
export _CSD_1_ClearSensors
export  CSD_1_ClearSensors
export _CSD_1_wReadSensor
export  CSD_1_wReadSensor
export _CSD_1_wGetPortPin
export  CSD_1_wGetPortPin
export _CSD_1_EnableSensor
export  CSD_1_EnableSensor
export _CSD_1_DisableSensor
export  CSD_1_DisableSensor
export _CSD_1_ScanSensor
export  CSD_1_ScanSensor
export _CSD_1_ScanAllSensors
export  CSD_1_ScanAllSensors
export _CSD_1_SetRefValue
export  CSD_1_SetRefValue

; Exported variables
export  CSD_1_bSensorNum
export _CSD_1_bSensorNum
export  CSD_1_waSnsResult
export _CSD_1_waSnsResult
export  CSD_1_wADC_Result
export _CSD_1_wADC_Result

export  CSD_1_bADCStatus
export _CSD_1_bADCStatus
...
;-----------------------------------------------
AREA bss
_CSD_1_waSnsResult:                             ; Current raw results
 CSD_1_waSnsResult:                     BLK  (2*CSD_1_TotalSensorCount)

AREA InterruptRAM (RAM, REL, CON)

 CSD_1_wADC_Result:
_CSD_1_wADC_Result:                     BLK 2

 CSD_1_bADCStatus:
_CSD_1_bADCStatus:                      BLK 1

 CSD_1_bScanTime:                       BLK 1

 CSD_1_bRefValue:                       BLK 1

 CSD_1_bBitMask:                        BLK  1  ; Temp bitmask byte
_CSD_1_bSensorNum:                              ; Current sensor to scan
 CSD_1_bSensorNum:                      BLK  1
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_Start(void)
;
;  DESCRIPTION:
...
_CSD_1_Start:
   ...
   mov   X, (CSD_1_TotalSensorCount-1)/8
.NullingCycle:
   mov   [X + <CSD_1_baSnsOnMask], 0
   dec   X
   jnc   .NullingCycle

   RAM_SETPAGE_IDX >CSD_1_baSnsDebounce
   mov   X, (CSD_1_TotalSensorCount-1)
   mov   A, CSD_1_Debounce
.NullDbnc:
...
.InitCycle:
   ...
   mov   [CSD_1_bNoiseThreshold], CSD_1_NOISE_THRESHOLD
   mov   [CSD_1_bNegativeNoiseThreshold], CSD_1_NEGATIVE_NOISE_THRESHOLD
   mov   [CSD_1_bBaselineUpdateThreshold], CSD_1_BASELINE_UPDATE_THRESHOLD
   mov   [CSD_1_bHysteresis], CSD_1_HYSTERESIS
   mov   [CSD_1_bDebounce], CSD_1_Debounce
   mov   [CSD_1_bLowBaselineReset], CSD_1_LowBaselineReset

   or    reg[CSD_1_ACE_CONTROL2_REG], 0x03  ; Start analog comparator

IF (CSD_1_CAPACITOR_PIN & CSD_1_CAPACITOR_P0_1)
...
ENDIF
   ...
   mov    [CSD_1_bRefValue], CSD_1_REF_VALUE

   mov    A, 2
   mov    X, 0xc
   call   CSD_1_SetScanMode

   M8C_SetBank1
   
IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P3_1)
   or    reg[PRT3DM0], 0x02
   or    reg[PRT3DM1], 0x02
   M8C_SetBank0
   or    reg[PRT3DM2], 0x02
...
IF (CSD_1_WITH_PWM)
   mov   reg[CSD_1_PWM_PERIOD_REG], CSD_1_PWM_PERIOD ; Set PWM period
   or    reg[CSD_1_PWM_CONTROL_REG], 0x01         ; Start PWM
ENDIF

   call  CSD_1_ClearSensors                           ; Loop through all sensors and deactivates them
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_SetScanMode
;
;  DESCRIPTION:
...
;  RETURNS:      None.
;  SIDE EFFECTS:
;    Uses CSD_1_wADC_Result variable as temporary registers
;
;-----------------------------------------------------------------------------
_CSD_1_SetScanMode:
 CSD_1_SetScanMode:
   ...
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR  0
   cmp   A, CSD_1_SLOW_SPEED + 1          ; if (bSpeed > SLOW_SPEED)  bSpeed = SLOW_SPEED
   jc    .SpeedOk
   mov   A, CSD_1_SLOW_SPEED
.SpeedOk:
   mov   [CSD_1_wADC_Result], A           ; _wADC_Result := bSpeed
   swap  A, X                                        ; A := bResolution
   cmp   A, CSD_1_MAX_RESOLUTION + 1      ; if (bResolution > MAX_RESOLUTION)  bResolution = MAX_RESOLUTION
   jc    .ResolutionOk
   mov   A, CSD_1_MAX_RESOLUTION
.ResolutionOk:

   mov   [CSD_1_wADC_Result+1], A         ; _wADC_Result+1 := bResolution

   sub   [CSD_1_wADC_Result+1], CSD_1_MIN_RESOLUTION   ; Remove offset - now Resolution code is from 0

   M8C_SetBank0

IF (CSD_1_WITH_PWM)
   and   reg[CSD_1_PRS_CONTROL_REG], ~0x01    ; stop the PRS8
   mov   reg[CSD_1_PRS_SEED_REG], 0x01        ; Seed := 1
   mov   reg[CSD_1_PRS_POLY_REG], CSD_1_PRS8_Poly
   or    reg[CSD_1_PRS_CONTROL_REG], 0x01     ; start the PRS8

ELSE
   and   reg[CSD_1_PRS_CONTROL_LSB_REG], ~0x01    ;stop the PRS16
   mov   reg[CSD_1_PRS_SEED_LSB_REG], 0x01        ; Seed := 1
   mov   reg[CSD_1_PRS_SEED_MSB_REG], 0x00

   mov   A, [CSD_1_wADC_Result+1]                 ; A := (bResolution-9)
   add   A, [CSD_1_wADC_Result]
   asl   A                                           ; A := byte offset in PRS Poly table
   mov   [CSD_1_bScanTime], A
   inc   A
   index CSD_1_PRS_Poly_Table             ; A := LSB poly
   mov   reg[CSD_1_PRS_POLY_LSB_REG], A
   mov   A, [CSD_1_bScanTime]
   index CSD_1_PRS_Poly_Table             ; A := MSB poly
   mov   reg[CSD_1_PRS_POLY_MSB_REG], A

   or    reg[CSD_1_PRS_CONTROL_LSB_REG], 0x01    ;start the PRS16
ENDIF

   mov   A, [CSD_1_wADC_Result+1]         ; A := bResolution
   index CSD_1_PWM_Table
   mov   reg[PWM_CR], A
   ...
   mov   A, [CSD_1_wADC_Result+1]         ; A := bResolution
   index CSD_1_VC3_Table
   mov   reg[OSC_CR3], A                             ; Set VC3

   mov   A, [CSD_1_wADC_Result]           ; A := bSpeed
   index CSD_1_VC1_Table
   mov   reg[OSC_CR1], A
   ...
   or    reg[CSD_1_CNT_OUT_REG], 0xC0     ; Use SysCLK direct
   jmp   .Continue

.NotUltraFast:
   mov   reg[CLK_CR3], 0x00                          ; Set VC1 as column clock source
   or    reg[CSD_1_CNT_OUT_REG], 0x40     ; Sync to SysClk
   and   reg[CSD_1_CNT_OUT_REG], ~0x80

.Continue:
   M8C_SetBank0

   mov   A, [CSD_1_bRefValue]
   call  CSD_1_SetRefValue
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_SetRefValue
;
;  DESCRIPTION:
...
;  RETURNS:      None.
;  SIDE EFFECTS:
;    Uses CSD_1_wADC_Result and CSD_1_bADCStatus
...
_CSD_1_SetRefValue:
 CSD_1_SetRefValue:
   ...
   mov   A, 8
.ValueOk:
   mov   [CSD_1_bRefValue], A
   add   A, 4
   mov   [CSD_1_bADCStatus], A

IF (CSD_1_WITH_PWM)
   mov   [CSD_1_wADC_Result + LSB], CSD_1_PWM_PERIOD
   mov   [CSD_1_wADC_Result + MSB], 0
   inc   [CSD_1_wADC_Result + LSB]
   adc   [CSD_1_wADC_Result + MSB], 0

   mov   X, [CSD_1_wADC_Result + LSB]
   mov   A, [CSD_1_wADC_Result + MSB]

.L0:
   dec   [CSD_1_bADCStatus]
   jz    .Ready
   swap  A, X
   add   [CSD_1_wADC_Result + LSB], A
   swap  A, X
...
.Ready:
   ...
   mov   A, 4
.L1:
   asr   [CSD_1_wADC_Result + MSB]
   rrc   [CSD_1_wADC_Result + LSB]
   dec   A
   jnz   .L1

   mov   A, [CSD_1_wADC_Result + LSB]
   jz    .L2
   dec   A
.L2:
   mov   reg[CSD_1_PWM_COMPARE_REG], A
ELSE
   mov   A, [CSD_1_bScanTime]
   inc   A
   index CSD_1_PRS_Seed_Table             ; A := LSB seed
   mov   [CSD_1_wADC_Result + LSB], A
   mov   X, A                                        ; V := LSB seed
   mov   A, [CSD_1_bScanTime]
   index CSD_1_PRS_Seed_Table             ; A := MSB seed
...
.Ready:
   and   reg[CSD_1_PRS_CONTROL_LSB_REG], ~0x01    ;stop the PRS16
   mov   A, [CSD_1_wADC_Result + LSB]
   mov   reg[CSD_1_PRS_SEED_LSB_REG], A
   mov   A, [CSD_1_wADC_Result + MSB]
   mov   reg[CSD_1_PRS_SEED_MSB_REG], A
   or    reg[CSD_1_PRS_CONTROL_LSB_REG], 0x01    ;start the PRS16
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_ClearSensors(void)
;
;  DESCRIPTION:
...
.InitLoop:                                              ; Loop through all sensors and deactivate
   push  A
   call  CSD_1_wGetPortPin
   call  CSD_1_DisableSensor
   pop   A
   dec   A
   jnc   .InitLoop
   RAM_EPILOGUE RAM_USE_CLASS_4
...
IF (CSD_1_WITH_PWM)
   and   reg[CSD_1_PWM_CONTROL_REG], ~0x01                           ; Stop PWM
   and   reg[CSD_1_PRS_CONTROL_REG], ~0x01                           ; stop the PRS8
ELSE
   and   reg[CSD_1_PRS_CONTROL_LSB_REG], ~0x01                       ; Stop the PRS16
ENDIF

   call  CSD_1_ClearSensors                                          ; Loop through all keys and deactivate
   and   reg[CSD_1_ACE_CONTROL2_REG], ~0x03                          ; Stop analog comparator
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_wGetPortPin(BYTE bSensor)
;
;  DESCRIPTION:
;      Returns the port and pin mask from a given key number.  This function
;      uses the table CSD_1_Sensor_Table to retreive this
;      information.
;-----------------------------------------------------------------------------
;
;  ARGUMENTS:
...
_CSD_1_wGetPortPin:
   ...
   index CSD_1_Sensor_Table2                     ; A contains the port value
   swap  A,X                                     ; Place port value in X and index*2 in A
   inc   A                                       ; Advance to next value (bitMask)
   index CSD_1_Sensor_Table2                     ; A contains bitMask, X has port number
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_EnableSensor(BYTE bSensorMask, BYTE bPort)
;
;  DESCRIPTION:
...
_CSD_1_EnableSensor:
   ...
   mov   [CSD_1_bBitMask],A                      ; Store bit mask in temp location
   push  X
   ...
   xor   [CSD_1_bBitMask],FFh                    ; Invert bitmask
   mov   A,reg[X+DM0_OFFSET]                     ; Clear DM0 bit to 0
   and   A,[CSD_1_bBitMask]
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_DisableSensor(BYTE bMask, BYTE bPort)
;
;  DESCRIPTION:
...
_CSD_1_DisableSensor:
   ...
   xor  [CSD_1_bBitMask],FFh                     ; Invert bitmask back to normal
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_ScanSensor(BYTE bStrtIdx)
;
;  DESCRIPTION:
...
_CSD_1_ScanSensor:
   ...
   mov   [CSD_1_bSensorNum], A
   call  CSD_1_wGetPortPin
   call  CSD_1_EnableSensor

   call  CSD_1_Precharge   
   ...
   mov   reg[INT_CLR1], ~CSD_1_CNT_INT_MASK
   mov   reg[CSD_1_CNT_PERIOD_REG], 0xFF
   CSD_1_Counter_EnableInt_M
   ...
   CSD_1_Counter_Start_M                ; Start counter
   or    reg[PWM_CR], 0x01                       ; Start Dedicated PWM

.L1:
   mov   A, [CSD_1_bADCStatus]
   jz    .L1

   M8C_DisableIntMask INT_MSK0, INT_MSK0_ACOLUMN_0 ; Disable ACol0 interrupt

   CSD_1_Counter_DisableInt_M
   CSD_1_Counter_Stop_M                 ; Stop counter
   ...
   mov   A, [CSD_1_wADC_Result + LSB]
   mov   [X+(CSD_1_waSnsResult+LSB)], A
   mov   A, [CSD_1_wADC_Result + MSB]
   mov   [X+(CSD_1_waSnsResult+MSB)], A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_ScanAllSensors(void)
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CSD_1_wReadSensor(BYTE bSensor)
;
;  DESCRIPTION:
...
_CSD_1_Precharge:
   ...
   M8C_SetBank1
IF (CSD_1_CAPACITOR_PIN & CSD_1_CAPACITOR_P0_1); Change CMOD to GND
   or    reg[PRT0DM0], 01h   
   and   reg[PRT0DM0], ~01h
ELSE
   or    reg[PRT0DM0], 08h   
...
IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P3_1) ; Change Rb to Strong
   or    reg[PRT3DM0], 02h
   and   reg[PRT3DM1], ~02h
   M8C_SetBank0
   and   reg[PRT3DM2], ~02h
...
.Pause:
   ...
   or    reg[PRT3DM2], 02h
ELSE
 IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P1_5); Change Rb to Strong
   or    reg[PRT1DM0], 20h
   and   reg[PRT1DM1], ~20h
   M8C_SetBank0
   and   reg[PRT1DM2], ~20h
...
.Pause:
   ...
   or    reg[PRT1DM2], 20h
 ELSE      
  IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P1_4); Change Rb to Strong
   or    reg[PRT1DM0], 30h
   and   reg[PRT1DM1], ~30h
   M8C_SetBank0
   and   reg[PRT1DM2], ~30h
...
.Pause:
   ...
   or    reg[PRT1DM2], 30h
  ELSE
   IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P1_0); Change Rb to Strong  
...
.Pause:
   ...
   or    reg[PRT1DM2], 01h  
   ELSE
    IF (CSD_1_FEEDBACK_PIN & CSD_1_FEEDBACK_P3_0); Change Rb to St
   or    reg[PRT3DM0], 01h
   and   reg[PRT3DM1], ~01h
   M8C_SetBank0
   and   reg[PRT3DM2], ~01h
